...
const createTree = function (folder, node) {
  // 여기에서 작업하세요.
  container.append(createTreeDom(folder));
...
function createTreeDom(obj) {
  ...
  for (let key in obj) {
    ...
    let div = '';
    if(obj[key].file !== undefined){
      obj[key].file.forEach((cur) => {
        div += `<li>${cur}</li>`;
      })
    }

    newSummary.innerHTML = obj[key].name + div;

    
    let childrenUl = createTreeDom(obj[key].folder);
    if (childrenUl) {
      newSummary.append(childrenUl);
    }
